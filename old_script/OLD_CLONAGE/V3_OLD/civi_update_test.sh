...
# Récupérer la première ligne du web,  STABLE et ce qu'il y'a entre parenthèses
# last_version_prod
# echo "Version de production disponible : X.XX.X (le numéro de la version récupéré quelque part sur downloads.civicrm.org)"
...
case $civi_type_version in
        "p")
            echo "Voulez-vous télécharger la dernière version X.XX.X en production ?"
            case $action in
                "o")
                    echo "Récupération de la dernière version X.XX.X en production ?"
                    echo "$last_version_prod"
                    $last_version_prod=civi_version
